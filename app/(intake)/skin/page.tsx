...
import { intakeNav, skinCopy, type SkinDepth } from '@/lib/copy/intake';
import { useIntake } from '@/lib/intake/store';

export default function SkinPage() {
  const router = useRouter();
  const stored = useIntake((s) => s.skinDepth);
  const setSkinDepth = useIntake((s) => s.setSkinDepth);
  const [value, setValue] = useState<SkinDepth | null>(stored ?? null);
  ...
  return (
    <div className="flex flex-col gap-10 pb-16">
      <section className="flex flex-col gap-4">
        ...
        <p className="text-base text-muted">{skinCopy.subhead}</p>
      </section>

      <TileSelect
        options={skinCopy.options}
        value={value}
        onChange={setValue}
      />
